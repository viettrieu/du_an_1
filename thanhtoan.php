...
if (isset($_GET['confirm'])) {
    try {
        $sql = "insert into orders(user_id,order_time,order_status) values('" . $_SESSION['user']['id'] . "','" . date("Y-m-d H:i:s") . "','Đang chờ');";
        $result = $conn->exec($sql);
        $order_id = $conn->lastInsertId();
        foreach ($_SESSION['carts'] as $product) {
            $sql = "insert into order_items(order_id,product_id,quantity) values('" . $order_id . "','" . $product['id']  . "','" . $product['quantity'] . "');";
            $conn->exec($sql);
            echo $sql;
        }
        ...
        echo 'đã xảu ra lỗi: ',  $e->getMessage(), "\n";
    }
}
?>
                        ...
                        <div class="col-6">
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Họ tên:</td>
                                        <td><b><?php echo $result['user_fullname'] ?></b></td>
                                    </tr>
                                    <tr>
                                        <td>Số điện thoại:</td>
                                        <td><b><?php echo $result['user_phone'] ?></b></td>
                                    </tr>
                                    <tr>
                                        <td>Email:</td>
                                        <td><b><?php echo $result['user_email'] ?></b></td>
                                    </tr>
                                    <tr>
                                        <td>Địa chỉ:</td>
                                        <td><b><?php echo $result['user_address'] ?></b></td>
                                    </tr>
                                </tbody>
                            </table>
                            <a href="?confirm" class="btn btn-primary px-4 py-2 w-100 text-white">Xác nhận thanh toán</a>
                        </div>
                    </div>
                </div>
            </div>
