...
class StatisticalModel extends DB
{
  ...
  {
    $sql = "SELECT DISTINCT book.id,book.title, thumbnail, book.price, detailed_order.published,COUNT(orderi.quantity) AS 'quantity'  FROM book INNER JOIN order_item orderi  ON book.id = orderi.productId INNER JOIN detailed_order ON detailed_order.id = orderi.orderId WHERE book.id IN (SELECT book_category.productId FROM book_category WHERE $cond) AND
  ...
  {
    $sql = "SELECT DISTINCT book.id,book.title, thumbnail, book.price, COUNT(wl.productId) AS 'quantity'  FROM book INNER JOIN wishlist wl  ON book.id = wl.productId WHERE book.id IN (SELECT book_category.productId FROM book_category WHERE $cond) GROUP BY book.id ORDER BY quantity DESC LIMIT $limit";
    return $this->pdo_query($sql);
  }
  public function cc()
  {
